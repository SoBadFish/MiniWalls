package org.sobadfish.miniwalls.tools;
...
public class Utils {
    ...
    public static MiniWallMain.UiType loadUiTypeByName(String name) {
        MiniWallMain.UiType type = MiniWallMain.UiType.AUTO;

        try{
          type = MiniWallMain.UiType.valueOf(name.toUpperCase());
    ...
    public static boolean copyFiles(File old,File target){
        ...
        if(files != null){
            for (File value : files) {
                MiniWallMain.sendMessageToConsole("复制地图中 ... "+((load / (float)files.length) * 100) +"%");
                load++;
                if (value.isFile()) {
                    // 复制文件
                    try {
